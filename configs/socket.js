...
function init(http) {
  ...
  status.on("connection", function(socket) {
    ...
    socket.on("subscribe", function(user, currentProject) {
      ...
      socket.join(
        currentProject ? currentProject.toString() : user.currentProject
      );
    ...
    socket.on("resubscribe", function(obj) {
      ...
      socket
        .leave(oldUser.currentProject)
        .join(currentProject ? currentProject : newUser.currentProject);
...
function unitAvailable(activityFlow, user) {
  ...
  var isUserInActivity = _.some(activityFlow.activity.contractors, function(
    contractor
  ) {
